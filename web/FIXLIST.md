...
> **Cập nhật kiểm tra thực tế:** 2026-07-11  
> **Tiến độ hiện tại:** khoảng **93% theo FIXLIST/code**, khoảng **88% production-ready**.  
> **Đã xác nhận mới nhất:** `web/backend` unit tests pass **21/21**, `web/frontend` build pass.  
> **Vừa cập nhật:** sửa route ordering `follow.routes.js` (`/alerts/read-all` đặt trước `/:id/read`) và đổi backend test scripts sang `cross-env` để chạy được trên Windows/Linux/macOS.

### Snapshot Trạng Thái Hiện Tại

#### Đã hoàn thành hoặc đã có trong code

- Joi validators và validation middleware cho các route write chính.
- Auth refresh token và change password endpoint.
- Service layer cho workspace, notification, collaboration, feedback, saved search.
- Async handler wrapper và controller refactor chính.
- System logger cho login/search/batch/system error.
- Auto-notification cho invite, workspace comment, sync job, follow paper match và email digest.
- Scheduler cho crawler queue, report refresh, daily/weekly follow digest.
- AI service gọi Gemini với fallback an toàn.
- Data cleaning pipeline và source mappers cho OpenAlex, Crossref, arXiv, IEEE, Semantic Scholar, ACM/Crossref, Exa.
- Frontend API client có refresh-token flow, logout, AI/search/library/follow/workspace/notification/feedback/admin APIs.
- Frontend production mode không fallback sample data mặc định.

#### Còn thiếu hoặc cần quyết định trước khi coi là 100%

- Chạy lại `npm run test:integration` khi có MongoDB/Docker local để xác nhận integration runtime.
- Chạy lại `npm run test:e2e` khi frontend/backend/Mongo chạy đầy đủ.
- Quyết định chính sách nguồn dữ liệu: code hiện vẫn có `ACM Digital Library`; nếu sản phẩm chốt bỏ ACM thì cần remove khỏi model, validators, scheduler, source health, seed và UI/source list.
- Dev mode vẫn cho phép sample fallback để làm UI nhanh; production đã tắt mặc định bằng `USE_SAMPLE_FALLBACK`.
- `FIXLIST.md` phần checklist chi tiết bên dưới là bản task gốc; snapshot này là trạng thái audit mới nhất theo code hiện tại.
...
|-------|--------|----------------|----------|
| BR-001 | Liên kết nguồn học thuật | ⚠️ OpenAlex, Semantic Scholar, Crossref, arXiv, IEEE, ACM/Crossref, Exa đã có code. Cần chốt lại có giữ ACM hay không | Quyết định sản phẩm |
...
| BR-004 | Lập lịch cập nhật tự động | ✅ Có `scheduler.service.js` chạy crawler queue/report refresh/follow digest | Không |
| BR-005 | Data cleaning | ✅ Có `paperCleaning.service.js` và importers dùng cleaning/dedupe pipeline | Không |
...
| BR-030 | Thông báo cập nhật | ✅ Có notification service, auto-create cho invite/comment/job/follow match và email digest | Không |
| BR-033 | AI tóm tắt | ✅ Có `ai.service.js` gọi Gemini với fallback an toàn | Không |
| BR-035 | AI giải thích thuật ngữ | ✅ Có endpoint/service gọi Gemini với fallback | Không |
| BR-036 | AI đề xuất hướng NC | ✅ Có endpoint/service gợi ý hướng nghiên cứu dựa trên gap data | Không |
| BR-038 | Quản lý tài khoản | ✅ Có refresh token endpoint, change password và frontend refresh flow | Không |
| BR-039 | RBAC | ✅ Middleware + Admin routes | Không |
| BR-041 | Giám sát nhật ký | ✅ Có `systemLogger` gắn login/search/batch/system error | Không |
