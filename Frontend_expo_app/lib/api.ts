...
import type { TimeRange, WidgetStatus, Kpi, TrendPoint, TrendSeries, GapCell, TrendingPaper, AiInsight, FollowedSubject, DashboardData, AxisOption } from '../data/types';
...
export type TrendRange = "12m" | "24m" | "5y" | "all";
...
export interface GrowthRow {
  ...
  cagr: number;
  trend: number[];
  status: "stable" | "growing" | "declining" | "emerging";
}
...
export interface WorkspaceItem {
  ...
  comments: any[];
  note: string;
  _populatedPaper?: PaperResult;
}

// Map GAP_ASPECTS and GAP_FIELDS since they were in gapSample
const GAP_ASPECTS: string[] = ["Lý thuyết", "Hiệu năng", "An toàn", "Khả năng giải thích"];
const GAP_FIELDS: {key: string; label: string; token: string}[] = [
  {key: "ml", label: "Machine Learning", token: "--c1"},
  {key: "cv", label: "Computer Vision", token: "--c2"},
  {key: "nlp", label: "NLP", token: "--c3"}
];
...
export type FollowType = "keyword" | "field" | "author";
...
export interface ResearcherProfile {
  ...
  institution?: string;
  fields?: string[];
  metrics?: { hIndex: number; citations: number; papers: number };
}
...
export async function storeAuth(result: AuthResult) {
...
export async function storeCurrentUser(user: AuthUser) {
...
async function refreshAuthTokens() {
  ...
  await storeAuth(payload.data);
  return payload.data.accessToken;
}

function redirectToLogin() { /* handle navigation in app */ }
...
export const userApi = {
  async updateProfile(data: { full_name: string; email: string }) {
    ...
    await storeCurrentUser(nextUser);
    return nextUser;
  },
};
